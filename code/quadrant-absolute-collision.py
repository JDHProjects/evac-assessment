...
class currentSnake(baseSnake.snake):
  ...
  def upCollision(self):
    ...
    return self.senseWall(up) or self.senseTail(up)

  def downCollision(self):
    down = [self.snake[0][0]+1,self.snake[0][1]]
    return self.senseWall(down) or self.senseTail(down)
  ...
  def getInputs(self):
      return [  snake_game.foodLeft(), snake_game.leftCollision(),
                ...
                snake_game.upCollision(), snake_game.foodAhead(),
                snake_game.downCollision(), snake_game.foodBehind()
                ]
